...
from .models import SellerUserRegistrationModel, FarmersCropsModels, SubsidyUserModel
...
def ViewSellerSubsidy(request):
    loginid=request.session['loginid']
    data= SellerUserRegistrationModel.objects.filter(name=loginid)
    return render(request,'sellers/SellerSubsidy.html',{'data':data})